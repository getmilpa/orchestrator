# Contributing to Milpa Orchestrator

Thanks for your interest in contributing! Milpa Orchestrator is the event-sourced
process engine of the Milpa framework — process definitions composed from
`milpa/workflow` states/transitions/gates, a pure `Reducer` that folds an append-only
`milpa/event-store` log into process state, an auto-advancing `ProcessRunner`, human-gate
decision surfaces via `milpa/live`, and three MCP tools (`process_instantiate`,
`process_list_pending_approvals`, `process_submit_decision`) that drive the loop.
...
- **Respect the tier boundary.** Milpa Orchestrator is the process engine — it
  composes `milpa/event-store`, `milpa/workflow`, `milpa/events`, `milpa/live` and
  `milpa/tool-runtime`, and sits near the top of the family. It depends on those
  packages, never the reverse. Do not introduce a dependency on Doctrine, a concrete
  container, or any product/plugin code, and do not push orchestrator concerns down
  into the packages it composes.
- **[Conventional Commits](https://www.conventionalcommits.org/)** — releases and
  ...
  interface or process contract is a `feat!:` / `BREAKING CHANGE:` (bumps MINOR
  while the package is `0.x`, MAJOR once it reaches `1.0`).

## Code style

The whole Milpa family (`milpa/core`, `milpa/http`, `milpa/tool-runtime`,
`milpa/orchestrator`) shares one coding standard, committed verbatim in every repo as
